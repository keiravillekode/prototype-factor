...
function gen_test_case(case)
    ...
    if prop == "append"
        ...
        return "{ $(format_nested_array(expected)) } [ $(l1) $(l2) $(word) ] unit-test"

    elseif prop == "concat"
        lists = format_nested_array(case["input"]["lists"])
        return "{ $(format_nested_array(expected)) } [ $(lists) $(word) ] unit-test"

    elseif prop in ("filter", "map")
        list = format_int_array(case["input"]["list"])
        func = FUNCTIONS[case["input"]["function"]]
        return "{ $(format_int_array(expected)) } [ $(list) $(func) $(word) ] unit-test"

    elseif prop in ("foldl", "foldr")
        list = format_int_array(case["input"]["list"])
        func = FUNCTIONS[case["input"]["function"]]
        return "{ $(to_int_str(expected)) } [ $(list) $(to_int_str(case["input"]["initial"])) $(func) $(word) ] unit-test"
    ...
    elseif prop == "reverse"
        ...
        return "{ $(format_nested_array(expected)) } [ $(list) $(word) ] unit-test"
    end
end
